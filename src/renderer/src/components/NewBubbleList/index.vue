<template>
  <div ref="scrollContainer" class="el-bubble-list" :class="{ 'always-scrollbar': props.alwaysShowScrollbar }" :style="{
...
<script setup>
import useScrollDetector from '@/utils/useScrollDetector.js'
...
function scrollToBottomImmediate() {
  try {
    if (scrollContainer.value && scrollContainer.value.scrollHeight) {
      // 临时禁用smooth滚动
      const originalScrollBehavior = scrollContainer.value.style.scrollBehavior
      ...
      nextTick(() => {
        if (scrollContainer.value) {
          ...
          setTimeout(() => {
            if (scrollContainer.value) {
              scrollContainer.value.style.scrollBehavior = originalScrollBehavior || 'smooth'
            }
          }, 100)
        }
      })
    ...
    console.warn('[BubbleList scrollToBottomImmediate error]: ', error)
    // 降级使用普通滚动
    scrollToBottom()
  }
}
